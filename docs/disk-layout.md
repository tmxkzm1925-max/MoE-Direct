...
| `repack\` next to your GGUF (`experts.bin`, `manifest.json`, `verify_report.json`) | once per model, after you approve the plan, on the default packed path | The direct-read expert store, its identity manifest, and the byte-level verification report that the serving gates consume - the repack output. Tens to hundreds of GB. An interrupted repack leaves `experts.bin.partial` here as well. **Never deleted automatically** - see [Update, reset, uninstall](#update-reset-uninstall). |
| `repack-virtual\` next to your GGUF (`manifest.json`, `plan_report.json`) | once per model, after you approve the plan, when you choose the virtual repack mode | The virtual repack output. There is no `experts.bin` here and no second copy of your experts: the manifest records where each routed slice already sits inside your GGUF, and `plan_report.json` records the plan that produced it. Measured here: about 6 MB for the 122B test model in 2 shards, and about 16 MB for the 397B 6-shard model. The size follows the number of routed slices, not the model's bytes. `repack-virtual` is the default folder name; the launcher passes the actual output path to the repacker with `--out`. **Never deleted automatically**, like the packed output. |
...
| Repack output | a `repack\` folder next to each of your GGUFs, or `repack-virtual\` for a virtual repack | **Not deleted automatically, ever.** The packed one is the big one - tens to hundreds of GB. Delete the folder to reclaim the space; the next run for that model will repack from scratch. A `repack-virtual\` folder holds only a manifest and its plan report, so deleting it reclaims very little; it costs a repack to rebuild all the same. |
...
| Where we are | Status |
|---|---|
| The repack that costs your disk the model's size a second time | **In this build as an opt-in, and not yet as the default.** Choosing the virtual repack mode writes a manifest instead of a second copy of the expert data, and the engine reads the experts out of your original file in place, so the space cost is the model plus a manifest and its plan report (measured at about 6 MB for the 122B test model and about 16 MB for the 397B one), and onboarding moves no expert data at all. It is an opt-in rather than the default because it does not hold read performance yet: experts scattered through the original file cost more to fetch than the packed layout does. The packed path is untouched and stays the default, and closing that speed gap is what v0.3.1 is for. |
